...
public class CodeController {
	...
	@RequestMapping(value = "/codeWriteSave.do") // [ 입력 저장처리 ] 
	public String insertCode(CodeVO vo) throws Exception{
		...
		if(result == null) { //이게 성공  다만, insert는 그냥 오류를 뱉는다.
			System.out.println("저장완료");
		}else {//이게 실패!!
			System.out.println("저장실패");
		}
		//UPDATE, DELETE 숫자 형태로 return(결과)를 숫자로 알려준다.-->실패 시(저장 안될 시)= 0
		return "redirect:codeList.do"; //이 페이지로 이동
	}
	
	@RequestMapping(value = "/codeList.do") // [ 목록 출력 ]
	...
	@RequestMapping(value = "/codeModifySave.do") // [ 수정 후 저장 ] 
